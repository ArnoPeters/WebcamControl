...
namespace MFCaptureD3D
{
	class CPreview : COMBase, IMFSourceReaderCallback, IDisposable
	{
		...
		public HResult SetDevice(MFDevice pDevice)
		{
			...
			lock (this)
			{
				try
				{
					...
					if (Succeeded(hr)) { hr = MFExtern.MFCreateAttributes(out pAttributes, 2); }

					if (Succeeded(hr)) { hr = pAttributes.SetUINT32(MFAttributesClsid.MF_READWRITE_DISABLE_CONVERTERS, 1); }

					if (Succeeded(hr)) { hr = pAttributes.SetUnknown(MFAttributesClsid.MF_SOURCE_READER_ASYNC_CALLBACK, this); }

					IMFSourceReader pRead = null;
					if (Succeeded(hr)) { hr = MFExtern.MFCreateSourceReaderFromMediaSource(pSource, pAttributes, out pRead); }

					if (Succeeded(hr)) { m_pReader = (IMFSourceReaderAsync) pRead; }
		...
		public HResult ResizeVideo()
		{
			...
			lock (this) { hr = m_draw.ResetDevice(); }
		...
		protected HResult TryMediaType(IMFMediaType pType)
		{
			...
			if (m_draw.IsFormatSupported(subtype)) { bFound = true; }
			else
			{
			...
			if (bFound) { hr = m_draw.SetVideoType(pType); }
				...
				try
				{
					if (Succeeded(hr))
					{
						if (pSample != null)
						{
							...
							if (Succeeded(hr)) { hr = m_draw.DrawFrame(pBuffer); }
						}
					}
