...
namespace MFCaptureD3D
{
	public class DrawDevice : COMBase
	{
		...
		private delegate void VideoConversion(IntPtr pDest, int lDestStride, IntPtr pSrc, int lSrcStride, int dwWidthInPixels, int dwHeightInPixels);
		...
		private RawRectangle m_rcDest; // Destination rectangle
		...
		public DrawDevice()
		{
			...
			//m_rcDest = //RawRectangle.Empty;

			VideoFormatDefs = new VideoFormatGUID[] {new VideoFormatGUID(MFMediaType.RGB32, TransformImage_RGB32), new VideoFormatGUID(MFMediaType.RGB24, TransformImage_RGB24), new VideoFormatGUID(MFMediaType.YUY2, TransformImage_YUY2), new VideoFormatGUID(MFMediaType.NV12, TransformImage_NV12)};
		...
		{
			if (m_pDevice == null) { return HResult.E_FAIL; }

			HResult hr = HResult.S_OK;

			// Check the current status of D3D9 device.
			Result r = m_pDevice.TestCooperativeLevel();
		...
		{
			RawRectangle rcSrc = new RawRectangle(0, 0, m_width, m_height);
			Rectangle rcClient = GetClientRect(m_hwnd);
			RawRectangle rectanClient = new RawRectangle(rcClient.Left, rcClient.Top, rcClient.Right - rcClient.Left, rcClient.Bottom - rcClient.Top);
		...
		{
			if (m_pDevice != null) { return HResult.S_OK; }
			...
			using (Direct3D d = new Direct3D()) { m_pDevice = new Device(d, 0, DeviceType.Hardware, hwnd, CreateFlags.HardwareVertexProcessing | CreateFlags.FpuPreserve | CreateFlags.Multithreaded, pp); }
		...
		public HResult SetVideoType(IMFMediaType pType)
		{
			...
			if (Succeeded(hr)) { m_PixelAR = PAR; }
			else { m_PixelAR.Numerator = m_PixelAR.Denominator = 1; }
		...
		{
			if (m_convertFn == null) { return HResult.MF_E_INVALIDREQUEST; }
			...
			if (m_pDevice == null || m_pSwapChain == null) { return HResult.S_OK; }
			...
			using (VideoBufferLock xbuffer = new VideoBufferLock(pCaptureDeviceBuffer))
			{
				...
				try
				{
					//using (dr.DataPointer)
					{
						// Convert the frame. This also copies it to the Direct3D surface.
						m_convertFn(dr.DataPointer, dr.Pitch, pbScanline0, lStride, m_width, m_height);
					}
				}
				finally
				{
					pSurf.UnlockRectangle();
			...
			m_pDevice.ColorFill(pBB, new RawColorBGRA(0x80, 0, 0, 0));

			// Blit the frame.
			RawRectangle r = new RawRectangle(0, 0, m_width, m_height);
			m_pDevice.StretchRectangle(pSurf, r, pBB, m_rcDest, TextureFilter.Linear);
			//res =
			//      hr = (HResult)res.Code;
		...
		public bool IsFormatSupported(Guid subtype)
		{
			for (int i = 0; i < VideoFormatDefs.Length; i++)
			{
				if (subtype == VideoFormatDefs[i].SubType) { return true; }
			}
		...
		// YUY2 to RGB-32
		//-------------------------------------------------------------------
		unsafe private static void TransformImage_YUY2(IntPtr pDest, int lDestStride, IntPtr pSrc, int lSrcStride, int dwWidthInPixels, int dwHeightInPixels)
		{
		...
		//
		//-------------------------------------------------------------------
		private static RawRectangle LetterBoxRect(RawRectangle rcSrc, RawRectangle rcDst)
		{
			int iDstLBWidth;
			int iDstLBHeight;

			if (MulDiv(rcSrc.Height(), rcDst.Height(), rcSrc.Height()) <= rcDst.Width())
			{
			...
			RawRectangle rc = new RawRectangle(left, top, iDstLBWidth, iDstLBHeight);
		...
		//-----------------------------------------------------------------------------
		private static RawRectangle CorrectAspectRatio(RawRectangle src, MFRatio srcPAR)
		{
			// Start with a rectangle the same size as src, but offset to the origin (0,0).
			RawRectangle rc = new RawRectangle(0, 0, src.Right - src.Left, src.Bottom - src.Top);
			...
			rc = new RawRectangle(0, 0, rcNewWidth, rcNewHeight);
			return rc;
		}
		...
		private static HResult GetDefaultStride(IMFMediaType pType, out int plStride)
		{
			...
			if (Failed(hr))
			{
				...
				hr = pType.GetGUID(MFAttributesClsid.MF_MT_SUBTYPE, out subtype);
				if (Succeeded(hr)) { hr = MFExtern.MFGetAttributeSize(pType, MFAttributesClsid.MF_MT_FRAME_SIZE, out width, out height); }
				...
				if (Succeeded(hr)) { hr = pType.SetUINT32(MFAttributesClsid.MF_MT_DEFAULT_STRIDE, lStride); }
			}
